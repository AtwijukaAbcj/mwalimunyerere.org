...
$uc_list = array('User','Administrator');
// URL and Database Configuration
if ($is_local) {
    define("SITE_URL", "http://localhost/mwalimunyerere.org/admin");
    define("HOME_URL", SITE_URL . "/dash");

    define("DB_HOST", "localhost");
    define("DB_USERNAME", "root");
    define("DB_PASSWORD", "");
    define("DB_NAME", "mwalimunyerere_admin");
} else {
    define("SITE_URL", "https://mwalimunyerere.org/admin");
    define("HOME_URL", SITE_URL . "/dash");

    define("DB_HOST", "localhost");
    define("DB_USERNAME", "mwalimunyerere_admin");
...
function generate_code(){
	global $dbh;
	$count = $dbh->query("SELECT COUNT(*) FROM requisitions ")->fetchColumn();
	$res = $count + 1;
	$ascode = "AST2022".$res;
	return $ascode;
}
...
function get_photos(){
	global $dbh;
	$result = $dbh->query("SELECT COUNT(*) FROM gallery ")->fetchColumn();
	return $result;
}

function get_contacts(){
	global $dbh;
	$result = $dbh->query("SELECT COUNT(*) FROM contacts ")->fetchColumn();
	return $result;
}
...
function get_branches(){
	$result = dbSQL("SELECT * FROM branches order by bid ");
	return $result;
}

function br_total(){
	global $dbh;
	$result = $dbh->query("SELECT COUNT(*) FROM branches order by bid ")->fetchColumn();
	return $result;
}

function de_total(){
	global $dbh;
	$result = $dbh->query("SELECT COUNT(*) FROM departments order by did ")->fetchColumn();
	return $result;
}

function as_total(){
	global $dbh;
	$result = $dbh->query("SELECT COUNT(*) FROM requisitions order by sid ")->fetchColumn();
	return $result;
}

function get_departments(){
	$result = dbSQL("SELECT * FROM departments order by did ");
	return $result;
}

function get_allotted(){
	$result = dbSQL("SELECT * FROM users WHERE role !='Administrator' order by userID ");
	return $result;
}
